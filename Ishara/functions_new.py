...
class PiPlus(Hadron):
    ...
    def sivers(self, kins, m1):  # adbar, bdbar): are not necessary
        '''
        ...
        QQ = kins[:, 3]
        a0 = self.A0(z, pht, m1)
        topleft = self.NN(x) * self.eu**2 * self.pdf(2, x, QQ) * self.ffPion(2, z, QQ)
        topright = self.NNanti(x) * self.edbar**2 * self.pdf(-1, x, QQ) * self.ffPion(-1, z, QQ)
...
class PiMinus(Hadron):
    ...
    def sivers(self, kins, m1): # aubar, bubar
        '''
        Calculate sivers assymetry for specified variables
        ...
        QQ = kins[:, 3]
        a0 = self.A0(z, pht, m1)
        topleft = self.NN(x) * self.ed**2 * self.pdf(1, x, QQ) * self.ffPion(1, z, QQ)
        topright = self.NNanti(x) * self.eubar**2 * self.pdf(-2, x, QQ) * self.ffPion(-2, z, QQ)
...
class PiZero(Hadron):
    ...
    def sivers(self, kins, m1): # aubar, bubar
        ...
        topleft = self.NN(x) * self.eu**2 * self.pdf(2, x, QQ) * (self.ffPion(2, z, QQ) + self.ffPion(2, z, QQ))
        # N_ubar * e_ubar^2 * f_ubar(x) * (D_pi+/ubar(z) + D_pi-/ubar(z))
        topright = self.NNanti(x) * self.eubar**2 * self.pdf(-2, x, QQ) * (self.ffPion(-2, z, QQ) + self.ffPion(-2, z, QQ))
...
class KPlus(Hadron):
    ...
    def sivers(self, kins, m1): # asbar, bsbar
        x = kins[:, 0]
        z = kins[:, 1]
        pht = kins[:, 2]
        QQ = kins[:, 3]
        ...
        topright = self.NNanti(x) * self.esbar**2 * self.pdf(-3, x, QQ) * self.ffKaon(-3, z, QQ)
        # e_u^2 * f_u(x) * (D_pi+/u(z) + D_pi-/u(z))
        bottomleft = self.eu**2 * self.pdf(2, x, QQ) * self.ffPion(2, z, QQ)
...
class KMinus(Hadron):
    ...
    def sivers(self, kins, m1): # asbar, bsbar
        ...
        topleft = self.NN(x) * self.es**2 * self.pdf(3, x, QQ) * self.ffKaon(3, z, QQ)
